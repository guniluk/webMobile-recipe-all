...
export default function LatestRecipe({ latestRecipe }) {
  ...
  return (
    <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-slate-800/80 bg-slate-900 group">
      <Link
        to={`/recipe/${latestRecipe.id}`}
        state={{ from: location.pathname }}
        className="block relative aspect-21/9 w-full min-h-75 overflow-hidden"
      >
        {/* Background Image */}
        <img
        ...
        {/* Overlay Gradients */}
        <div className="absolute inset-0 bg-linear-to-t from-slate-950 via-slate-900/60 to-transparent md:bg-linear-to-r md:from-slate-950 md:via-slate-950/70 md:to-transparent" />
